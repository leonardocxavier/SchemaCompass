...
mysql "$MYSQL_DB" -N -e "SHOW TABLES" | while read -r TABELA; do
    ...
    echo "--------------------------------------------------"
done # Fim do loop principal da Parte 1

echo "--- SUCESSO! PARTE 1 CONCLUÍDA. Mapa semântico gerado em '$ARQUIVO_MAPA'. ---"
echo ""

# ==============================================================================
# SOLUÇÃO: Forçar a sincronização do buffer para o disco
# Adicione este bloco para garantir que o arquivo da Parte 1 esteja 100% escrito.
# ==============================================================================
echo "Sincronizando dados com o disco para garantir a integridade do arquivo..."
sync
